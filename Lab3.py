...
def RBT(filename):
    ...
    with open(filename) as file:
        for line in file:
            ...
            rbt_tree.insert(node)
            line = file.readline()

    return rbt_tree
        
def print_anagrams(word, english_words, prefix=""):
    #Prints the anagrams only if there are in the tree
    ...
    if len(word) <= 1:
        ...
        for i in range(len(word)):
            ...
            if cur not in before: # Check if permutations of cur have not been generated.
                count_anagrams(english_words, before + after, prefix + cur)  
    return count
...
def max_anagrams(filename):
    ...
    max_word = ""
    for line in file:
        count = count_anagrams(english_words,line[0:-1])  # -1 index means last one
        print(line[0:-1], count)
        if count > max_count:
            max_count = count
            max_word = line[0:-1]
    print("The word with the most anagrams stored in %s was %s with %d anagrams" % (filename, max_word, max_count))     
...
def main():
    ...
    if tree_type == "1":
        avl_word_tree = AVL(filename)
        anagram_word = input("Which word do you want to permutate?")
        print_anagrams(anagram_word, avl_word_tree)
        permutation_count = count_anagrams(avl_word_tree, anagram_word)
        print("Number of permutations of the word: ", permutation_count)
        greatest_anagram_word = greatest_word(anagram_word, avl_word_tree)
        print("Word with more anagrams: " + greatest_anagram_word)
        
    elif tree_type == "2":
        rbt_word_tree = RBT(filename)
        anagram_word = input("Which word do you want to permutate?")
        print_anagrams(anagram_word, rbt_word_tree)
        permutation_count = count_anagrams(rbt_word_tree, anagram_word)
        print("Number of permutations of the word: ", permutation_count)
        greatest_anagram_word = greatest_word(anagram_word, rbt_word_tree)
        print("Word with more anagrams: " + greatest_anagram_word)
